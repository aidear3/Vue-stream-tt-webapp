    ...
    computed: {
        getIframeUrl() {
            if(this.$router.currentRoute.value.query.resume) {
                this.iframeSrc = `${PLAYER_URL}player.php?series=` + this.$route.params.playerId + "&about=1&streams=1&videos=1&channels=1&matches=1&svods=1&apps=0&chat=1&donation=1&epg=0&resume=" + this.$router.currentRoute.value.query.resume;
            } else {
