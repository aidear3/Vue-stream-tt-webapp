  ...
  computed: {
    ...
    FeaturedItems() {
      ...
      if (this.$route.params.subMenuId) {
        ...
        let data = MainMenuTypes[this.$route.params.menuId].sub_menu_items.find(arr => arr.id == this.$route.params.subMenuId);
        return data.length ? data.featured_items : [];

      } else
        return MainMenuTypes[this.$route.params.menuId]?.featured_items;
    },
  },
  watch: {
    Categories(newVal) {
